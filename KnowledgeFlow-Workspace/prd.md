...
6. As a researcher, I want to save a clip with a single button press, so that the entire capture takes less than two seconds of my attention.
7. As a researcher, I want to see a loading spinner while the plugin processes the routing, so that I have visual confirmation the system is working.
8. As a researcher, I want a success toast after saving that shows the matched note name and, when LLM validation is enabled, the AI's one-sentence routing justification, so that I can immediately verify the routing decision.
9. As a researcher, I want an error toast with clear retry guidance if the connection to `localhost` fails or if the Gemini API rate limit is hit, so that I understand what went wrong and how to resolve it.
10. As a researcher, I want to dismiss the popup by clicking outside it or pressing Escape, so that it never permanently blocks my reading.

### Extension Popup & Options

11. As a researcher, I want to open the extension action popup from the browser toolbar and see my ten most recent clips fetched live from the Obsidian plugin, so that I can glance at what I have saved without opening Obsidian.
12. As a researcher, I want the extension popup to clearly display an "Obsidian Offline" state when the localhost server is unreachable, so that I always understand system status.
13. As a researcher, I want an extension options page where I configure the `localhost` port and paste my local authorization token, so that the extension securely pairs with my active Obsidian vault.
14. As a researcher, I want a "Test Connection" button on the options page that validates live communication with the local Obsidian server, so that I know the extension is correctly configured before I start clipping.

### Obsidian Plugin — Differential Vault Sync & Indexing

15. As an Obsidian user, I want the plugin to perform a differential metadata sync on startup by checking modified timestamps (`mtime`) against a local cache, so that only new or edited notes are re-embedded and my daily API quota is not wasted on unchanged content.
16. As an Obsidian user, I want the initial indexing of a large vault to be processed in sequential batches of 100 notes per API call to the batch embedding endpoint, so that my personal Gemini free-tier daily quota is never exhausted during setup.
17. As an Obsidian user, I want note changes to update the local vector index within 5 seconds of a file save, so that the index is constantly fresh for routing.
18. As an Obsidian user, I want deleted notes to be purged from the local vector index instantly on the `vault.on('delete')` event, so that the system never routes clips to dead links.
19. As an Obsidian user, I want a sync status indicator in Obsidian's bottom status bar showing a live icon (✅ synced / ⏳ syncing / ⛔ offline) with the last index timestamp, cached note count, and remaining daily API quota estimate visible on hover, so that I can monitor index health at a glance.
20. As an Obsidian user, I want an auto-sync toggle in the plugin settings, so that I can disable all background embedding activity when working completely offline.

### Obsidian Plugin — Authentication & Configuration

21. As an Obsidian user, I want to generate a local authorization token from the plugin settings with one click, so that I can securely pair the Chrome Extension with my vault without configuring external credentials.
22. As an Obsidian user, I want to enter my personal Google AI Studio (Gemini) API key directly into the plugin settings, so that all AI operations run inside my own free-tier limits.
23. As an Obsidian user, I want the plugin to validate my Gemini API key on entry via a test embedding call and display immediate success or failure visual feedback, so that I can catch misconfiguration before attempting to use the system.
24. As an Obsidian user, I want an LLM Validation toggle in the plugin settings, so that I can disable the Flash routing justification call to save API quota when I trust the embedding-only routing.

### Intelligent Clip Insertion

25. As an Obsidian user, I want saved clips to be written directly to my local Markdown files using Obsidian's native `app.vault.process()` API, so that safe, concurrent-write-safe modifications are guaranteed and never collide with background cloud sync providers.
26. As an Obsidian user, I want each clip inserted immediately after the paragraph most semantically similar to the clip, determined through a header-then-paragraph drill-down using array-batched embedding calls, so that the clip lands in a contextually precise location without exhausting my API quota.
27. As an Obsidian user, I want each inserted clip formatted as an Obsidian `[!quote]` callout block containing the clipped text, a hyperlinked source URL, an ISO timestamp, and any user-provided tags as Dataview-compatible inline fields, so that clips are natively rendered, searchable, and queryable.
28. As an Obsidian user, I want clips whose top cosine similarity score falls below 0.70 (configurable) to automatically generate a new note with an AI-generated title and an implicit `#kf-inbox` tag, so that low-confidence clips are safely captured for later review rather than misrouted.
29. As an Obsidian user, I want the auto-generated note title to be produced within the same Flash validation call (via an optional `suggestedTitle` field in the JSON response), so that no additional API call is consumed for this operation.

### Rate Limit Handling

30. As an Obsidian user, I want the plugin to accept an incoming clip immediately and queue it when the Gemini rate limit is hit, so that the capture is never lost.
31. As a researcher, I want a persistent countdown banner in the extension popup and a Notice in Obsidian showing the retry timer when a clip is queued due to rate limiting, so that I understand the system is working and when to expect completion.
32. As an Obsidian user, I want queued clips to be retried automatically after the rate-limit reset window, so that I never have to manually intervene.

### Clip History — Extension & Obsidian Views

33. As a researcher, I want the extension action popup to fetch and display my ten most recent clips from the Obsidian plugin via `GET /clips`, so that clip history is always sourced from a single source of truth.
34. As an Obsidian user, I want to open a searchable, filterable "Clip History" leaf view inside Obsidian to review all past clips in one place.
35. As an Obsidian user, I want each history row to feature an **Undo / Relocate** button, so that I can instantly pull a clip out of an incorrect note and move it to the right destination if the AI routing made a mistake.
36. As an Obsidian user, I want the Undo action to remove the callout block from the target file using `vault.process`, searching by the clip's unique embedded ID, so that removal is always safe and precise regardless of subsequent edits to the file.
37. As an Obsidian user, I want the Relocate action to open Obsidian's built-in fuzzy-note-search modal so I can select the correct destination note with minimal friction.
...
- **Popup rendering:** The content script injects a floating popup element wrapped in a **Shadow DOM** to fully isolate styles from the host page. This is the only path that meets the 200 ms appearance SLA.
...
- The LLM validation toggle (User Story 24) also controls the auto-title generation pathway: when disabled, new notes created for below-threshold clips use `page_title + ISO timestamp` as their filename, since the Flash call that produces `suggestedTitle` is skipped.
